...
<template>
   <div class="flex flex-col gap-6">
      ...
      <PageHeader title="个人资料" subtitle="管理您的账号信息和安全设置">
         <template #actions>
            <template v-if="isEditing">
               <Button label="取消" severity="secondary" outlined @click="cancelEditing" />
               <Button
                  label="保存"
                  icon="pi pi-check"
            ...
            <template v-else>
               <Button label="编辑资料" icon="pi pi-pencil" @click="startEditing" />
            </template>
         </template>
      </PageHeader>
      ...
      <!-- Loading State -->
      <div v-if="isLoadingProfile" class="flex items-center justify-center py-24">
      ...
      <div v-else-if="profile" class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
         ...
         <div class="flex flex-col gap-6">
            ...
            >
               ...
               >
                  <div
                     class="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,rgba(255,255,255,0.1),transparent)]"
                  ></div>
               </div>
               ...
               <div class="px-6 pb-6">
                  <div class="flex items-end gap-5 -mt-12 relative z-10">
                     ...
                     <div class="flex-1 pb-1">
                        <div class="flex items-center gap-3 mb-1">
                           <h2 class="text-xl font-bold text-surface-900 dark:text-surface-100 m-0">
                              {{ profile.name }}
                           </h2>
                           <Tag
                  ...
                  <div class="grid gap-5 sm:grid-cols-2">
                     <div class="flex flex-col gap-2">
                        <label class="text-sm font-medium text-surface-500"> 学号 / 工号 </label>
                        <InputText :modelValue="profile.studentId" disabled class="w-full" />
                     </div>
                     <div class="flex flex-col gap-2">
                        <label class="text-sm font-medium text-surface-500"> 邮箱 </label>
                        <InputText :modelValue="profile.email" disabled class="w-full" />
                     </div>
                     <div class="flex flex-col gap-2">
                        <label class="text-sm font-medium text-surface-700 dark:text-surface-300">
                           显示名称
                        </label>
                        <InputText v-if="isEditing" v-model="editForm.displayName" class="w-full" />
                        <InputText
                           v-else
                     ...
                     <div class="flex flex-col gap-2">
                        ...
                        <InputText v-if="isEditing" v-model="editForm.phone" class="w-full" />
                        <InputText v-else :modelValue="profile.phone" disabled class="w-full" />
                     </div>
                     <div class="flex flex-col gap-2 sm:col-span-2">
                        <label class="text-sm font-medium text-surface-700 dark:text-surface-300">
                           个人简介
                        </label>
                        <Textarea v-if="isEditing" v-model="editForm.bio" rows="3" class="w-full" />
                        <Textarea
                           v-else
               ...
               <div v-else class="flex flex-col gap-3">
                  ...
                  >
                     <div
                        ...
                        "
                     >
                        <i :class="record.status === 'success' ? 'pi pi-check' : 'pi pi-times'"></i>
                     </div>
         ...
         <div class="flex flex-col gap-6">
            ...
            >
               ...
               <div class="flex flex-col gap-4">
                  ...
                  <div class="flex items-center justify-between py-2">
                     ...
                     <span class="text-xs font-mono text-surface-400 dark:text-surface-500">
                        {{ profile.id }}
                     </span>
                  </div>
               </div>
            ...
            >
               ...
               <div class="flex flex-col gap-4">
                  ...
                  <div v-if="isLoadingSettings" class="flex items-center justify-center py-4">
                     <i class="pi pi-spin pi-spinner text-xl text-surface-400"></i>
                  </div>
                  <template v-else-if="securitySettings">
                     ...
                     >
                        <div>
                           ...
                           <div class="text-xs text-surface-500">接收重要通知邮件</div>
                        </div>
                        <ToggleSwitch
                     ...
                     >
                        <div>
                           ...
                           <div class="text-xs text-surface-500">新设备登录时发送提醒</div>
                        </div>
                        <ToggleSwitch
      ...
      >
         <div class="flex flex-col gap-5">
            ...
            <div class="flex flex-col gap-2">
               ...
               <div v-if="passwordForm.newPassword" class="flex items-center gap-2">
                  <div
                     class="flex-1 h-1.5 bg-surface-200 dark:bg-surface-700 rounded-full overflow-hidden"
                  >
                     <div
                  ...
                  <span class="text-xs text-surface-500 w-12">{{ passwordStrengthLabel }}</span>
               </div>
            </div>
            <div class="flex flex-col gap-2">
               <label class="text-sm font-medium text-surface-700 dark:text-surface-300">
         ...
         <template #footer>
            <div class="flex justify-end gap-3">
               <Button label="取消" severity="secondary" outlined @click="passwordDialog = false" />
               <Button
                  label="确认修改"
                  :disabled="
                     !passwordForm.currentPassword ||
      ...
      >
         <div class="flex flex-col items-center gap-6">
            <div class="w-32 h-32 rounded-2xl overflow-hidden bg-surface-100 dark:bg-surface-700">
               <img
                  v-if="profile"
                  :src="profile.avatar"
                  :alt="profile.name"
