...
watch(
   ...
   isOpen => {
      if (!isOpen || !aside.value) return
      const cleanup = onClickOutside(aside, () => {
         sidebarStore.closeMobile()
         cleanup()
      })
   }
...
<template>
   <aside
      ref="aside"
      class="fixed left-0 top-0 bottom-0 z-1000 transition-all duration-300 ease-in-out max-lg:w-68! max-lg:-translate-x-full"
      :class="{
   ...
   >
      ...
      >
         <!-- Logo 区域 -->
         <div class="p-6 px-4 border-b border-surface-100 dark:border-surface-800">
            <div
            ...
            >
               ...
               <Transition name="fade">
                  ...
                  >
                     ...
                     <span class="text-xs text-surface-500 tracking-wider"> 认证中心 </span>
                  </div>
               </Transition>
            </div>
         </div>
         ...
         <nav class="flex-1 p-4 px-3 overflow-y-auto overflow-x-hidden">
            <ul class="list-none m-0 p-0 flex flex-col gap-1">
               <li v-for="item in menuItems" :key="item.label" class="relative group">
                  ...
                  >
                     <span class="shrink-0 w-6 flex items-center justify-center text-lg">
                        <i :class="item.icon"></i>
                     </span>
                     <Transition name="fade">
                        <span
            ...
            >
               ...
               <Transition name="fade">
                  ...
                  >
                     ...
                     <span class="text-xs text-surface-500"> {{ authStore.userRole }} </span>
                  </div>
               </Transition>
            </div>
         </div>
