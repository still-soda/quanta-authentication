...
func NewDashboardHandler(
	...
	return &DashboardHandler{
		...
		oauthSrv:   oauthSrv,
		logger:     logger.With("handler", "DashboardHandler"),
	}
}
...
func (h *DashboardHandler) GetDashboardStats(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get total user count", "error", err.Error())
	...
	if err != nil {
		h.logger.Error("failed to get total user trend", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	...
	if err != nil {
		h.logger.Error("failed to get recent auth user count", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	userAuthCnt, err := h.cache.GetKeyValueAsInt64("todays-authcount")
	if err != nil {
		h.logger.Error("failed to get todays auth count", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	...
	if err != nil {
		h.logger.Error("failed to get recent oauth app count", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	oauthAppCnt, err := h.oauthSrv.CountClients()
	if err != nil {
		h.logger.Error("failed to get weekly oauth app count", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	...
	if err != nil {
		h.logger.Error("failed to get active user count", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	maxActiveTrend, err := h.counterSrv.GetRecentCounter(string(config.CounterDailyMaxActiveUser), 9, &services.ZERO)
	if err != nil {
		h.logger.Error("failed to get active user trend", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
...
func (h *DashboardHandler) GetUserDistributionByRole(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get user distribution by role", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
...
func (h *DashboardHandler) GetAuthTrend(c *gin.Context) {
	...
	case "monthly":
		days = 30
	default:
		h.logger.Error("invalid range type for auth trend", "range", rangeType)
	...
	if err != nil {
		h.logger.Error("failed to get auth user trend", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
	...
	if err != nil {
		h.logger.Error("failed to get today's auth user count", "error", err.Error())
		response.HandlerError(c, e.ErrInternalServerError)
		return
	}
